...
(:action drive-buy-all
 :parameters (?t - truck ?from - place ?g - goods ?m - market)
 :precondition (and (loc ?t ?from)  (> (on-sale ?g ?m) 0)  (<= (on-sale ?g ?m) (- (request ?g) (bought ?g)))   )
 :effect (and (assign (on-sale ?g ?m) 0) (not (loc ?t ?from)) (loc ?t ?m) (increase (total-cost) (+ (drive-cost ?from ?m) (* (on-sale ?g ?m) (price ?g ?m)))) 
                  (increase (bought ?g) (on-sale ?g ?m)))) 
                  
                  
                  	    
 (:action drive-buy-allneeded
 :parameters (?t - truck ?from - place ?g - goods ?m - market)  
 :precondition (and  (loc ?t ?from)  (> (on-sale ?g ?m) 0) (> (on-sale ?g ?m) (- (request ?g) (bought ?g))) )
 :effect (and (not (loc ?t ?from)) (loc ?t ?m)
	         (increase (total-cost) (+ (drive-cost ?from ?m) (* (- (request ?g) (bought ?g)) (price ?g ?m))))
