...
(:action calibrate-take_image
:parameters (?r - rover ?i - camera ?t - objective ?o - objective ?w - waypoint ?m - mode)
:precondition (and (at ?r ?w) (>= (energy ?r) 3) (on_board ?i ?r) (calibration_target ?i ?t) (equipped_for_imaging ?r) (visible_from ?t ?w) (visible_from ?t ?w) (supports ?i ?m))
:effect (and (have_image ?r ?o ?m) (not (calibrated ?i ?r)) (decrease (energy ?r) 3)))




(:action sample_soil-drop
:parameters (?x - rover ?s - store ?p - waypoint)
:precondition (and  (>= (energy ?x) 3) (empty ?s) (equipped_for_soil_analysis ?x) (at_soil_sample ?p) (at ?x ?p) (store_of ?s ?x))
:effect (and (not (full ?s)) (empty ?s) (decrease (energy ?x) 3) (have_soil_analysis ?x ?p) (not (at_soil_sample ?p))))




(:action sample_rock-drop
:parameters (?x - rover ?s - store ?p - waypoint)
:precondition (and (at ?x ?p) (store_of ?s ?x) (empty ?s) (equipped_for_rock_analysis ?x) (>= (energy ?x) 5) (at_rock_sample ?p))
:effect (and (not (full ?s)) (empty ?s) (decrease (energy ?x) 5) (have_rock_analysis ?x ?p) (not (at_rock_sample ?p)))
)
