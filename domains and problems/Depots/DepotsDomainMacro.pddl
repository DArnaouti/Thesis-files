...
(:action Lift-Load
 ...
 :precondition (and (located ?x ?p) (available ?x) (located ?y ?p) (on ?y ?z) (clear ?y) (located ?x ?p) (located ?t ?p)
                    (<= (+ (current_load ?t) (weight ?y)) (load_limit ?t)))
 :effect (and (not (lifting ?x ?y)) (in ?y ?t) (available ?x)  (increase (current_load ?t) (weight ?y)) 
           (not (located ?y ?p))  (not (clear ?y)) (clear ?z) (not (on ?y ?z)) (increase (fuel-cost) 1)) 
 
 )
 
 (:action Unload-Drop
  :parameters (?x - hoist ?y - crate ?t - truck ?z - surface ?p - place)
  :precondition (and (located ?x ?p) (located ?t ?p) (available ?x) (in ?y ?t) (located ?z ?p) (clear ?z) )
