...
(:functions 
	...
	(fuel-cost)
              ...
              (:action Lift-Load-Drive
:parameters (?x - hoist ?y - crate ?z - surface ?t - truck ?p1 - place ?p2 - place)
:precondition (and (at ?x ?p1) (available ?x) (at ?y ?p1) (on ?y ?z) (clear ?y) (at ?t ?p1) (<= (+ (current_load ?t) (weight ?y)) (load_limit ?t)))
:effect (and (not (at ?y ?p1)) (not (at ?t ?p1)) (not (lifting ?x ?y)) (not (clear ?y)) (available ?x) (in ?y ?t) (at ?t ?p2)
            (clear ?z) (not (on ?y ?z)) (increase (fuel-cost) 11) (increase (current_load ?t) (weight ?y)))) 
...
(:action Lift-Load
 ...
 :precondition (and (at ?x ?p) (available ?x) (at ?y ?p) (on ?y ?z) (clear ?y) (at ?t ?p)
