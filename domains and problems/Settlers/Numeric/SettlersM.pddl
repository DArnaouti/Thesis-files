...
(:action load-move-cart
   :parameters (?v - vehicle ?p1 - place ?p2 - place ?r - resource) 
   :precondition (and (is-at ?v ?p1) 
		      (> (available ?r ?p1) 0) 
		      (> (space-in ?v) 0)
              (is-cart ?v)
...
(:action build-cabin-fell-timber 
   :parameters (?p - place) 
   :precondition (woodland ?p) 
   :effect (and (increase (labour) 2) (has-cabin ?p) (increase (available timber ?p) 1)) )
 
 
(:action build-quarry-break-stone 
   :parameters (?p - place) 
   :precondition (mountain ?p) 
   :effect (and (increase (labour) 3) (has-quarry ?p) (increase (available stone ?p) 1) (increase (resource-use) 1)))
   
   
   
   
 (:action build-coal-stack-burn-coal 
   :parameters (?p - place) 
...
(:action build-sawmill-saw-wood 
   :parameters (?p - place) 
   :precondition (>= (available timber ?p) 3) 
   :effect (and (decrease (available timber ?p) 3) 
		(has-sawmill ?p)
...
(:action build-ironworks-make-iron 
   :parameters (?p - place) 
